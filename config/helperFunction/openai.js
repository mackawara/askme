...
const openAiCall = async (chatID, tokenLimit, prompt) => {
  ...
  const isTokenUser = await redisClient.hGet(chatID, 'isTokenUser') == '1' ? true : false
  ...
  try {
    ...
    if (response) {
      if ("choices" in response) {
        ...
        redisClient.hIncrBy(chatID, 'availableTokens', -response.usage.total_tokens)
        ...
        console.log(`Usage tokens are as follows \n token limit = ${tokenLimit} total usage=${response.usage.total_tokens}\n INput tokens = ${response.usage.prompt_tokens}\n Completion tokens = ${response.usage.completion_tokens}`)
        user.save();
        totalUsage.save();
